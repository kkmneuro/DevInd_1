...
//05/03/2012	NAMO         1.Code for columnProfile
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
namespace PALSA.Frm
{
    public partial class frmOrderBook : frmBase
    {
        ...
        private void frmOrderBook_Load(object sender, EventArgs e)
        {
            ...
            uctlOrderBook1.ui_uctlGridOrderBook.ui_ndgvGrid.Scroll +=new ScrollEventHandler(ui_ndgvGrid_Scroll);
            ...
            var columnsArray = new DataGridViewColumn[49];
            ...
            columnsArray[41] = ClsCommonMethods.CreateGridColumn(columnsArray[41], "clmProfitCurrency",
            ...
            columnsArray[45].DisplayIndex = 1;    
            ...
            if (ClsGlobal.MarketMakerAccountId > 0)
            {
            ...
            uctlOrderBook1.SelectionCriteriaChanged += uctlOrderBook1_SelectionCriteriaChanged;
            uctlOrderBook1.OnAccountChanged += uctlOrderBook1_OnAccountChanged;
            //uctlOrderBook1.OnSearchClick += new Action<int, string, string, string, string, DateTime, DateTime>(uctlOrderBook1_OnSearchClick);

            //foreach (DataRow item in Cls.clsTWSOrderManager.INSTANCE.orderHistoryDS.dtOrderHistory.Rows)
            //{
            //    int index = uctlOrderBook1.ui_uctlGridOrderBook.Rows.Add(
            //      item["ClientOrderID"], item["Account"], item["ProductType"], item["Product"], item["Contract"],
            //      item["ExpireDate"], item["OrderType"], item["Side"], item["CumQty"], item["LeavesQty"], item["Price"],
            //      item["OrderStatus"], item["TimeInForce"], "", "", item["AvgPrice"], "", "", "", "", "", "", "", "", "", "", "", ""
            //      , "", "", "", "", "", "", "", "", "", "");
            //    uctlOrderBook1.ui_uctlGridOrderBook.Rows[index].DefaultCellStyle.BackColor = Color.FromName(item["Color"].ToString());
            //    uctlOrderBook1._DDRow.Add(Convert.ToInt32(item["ClientOrderID"]), item);
            //}
...
void  ui_ndgvGrid_Scroll(object sender, ScrollEventArgs e)
{
    ...
    //uctlOrderBook1.ui_nsbFilterValue.Text = Convert.ToString(e.NewValue) + " : " + Convert.ToString(e.OldValue);
}
        ...
        //{
        //    uctlOrderBook1.ui_uctlGridOrderBook.DataSource = Cls.clsTWSOrderManager.INSTANCE.orderHistoryDS.dtOrderHistory;
        //}
        ...
        private void AddOrdersToDataSet(OrderHistory item)
        {
            DataRow dr = clsTWSOrderManagerJSON.INSTANCE.orderHistoryDS.dtOrderHistory.NewRow();
            int digit = 0;
            string productType = ClsGlobal.DDReverseProductType[Convert.ToSByte(item.ProductType)];
            ...
            else if (productType == "AU")
            {
                pType = "AUCTION";
            }
            else if (productType == "BON")
            {
                pType = "BOND";
            }
          
            //InstrumentSpec objInstrumentSpec = ClsTWSContractManager.INSTANCE.GetContractSpec(item.Contract, pType,
            //                                                                                  item.Product);
            InstrumentSpec objInstrumentSpec = ClsTWSContractManager.INSTANCE.ddContractDetails[item.Contract];
            if (objInstrumentSpec != null)
                digit = objInstrumentSpec.Digits;
            dr["Account"] = item.Account;
            //dr["Author"] = item.author;
            dr["AvgPrice"] = item.AvgPx; // / Math.Pow(10, digit);
            dr["ClientOrderID"] = item.ClOrdId;
            dr["Contract"] = item.Contract;
            dr["CumQty"] = item.CumQty;
            dr["OrderType"] = ClsGlobal.DDReverseOrderType[Convert.ToSByte(item.OrderType)];
            //dr["ExecID"] = item.ExecID;
            //dr["ExecTransType"] = item.ExecTransType;
            //dr["ExecType"] = item.ExecType;
            //dr["ExpireDate"] = item.ExpireDate;
            dr["Industry"] = item.industry;
            dr["LeavesQty"] = item.LeavesQty;
            dr["OrderID"] = (int)item.OrderID;

            dr["OrderStatus"] = ClsGlobal.DDReverseOrderStatus[(sbyte)item.OrderStatus];
            if (ClsGlobal.DDReverseOrderStatus[Convert.ToSByte(item.OrderStatus)].ToUpper().Contains("FILLED") ||
                ClsGlobal.DDReverseOrderStatus[Convert.ToSByte(item.OrderStatus)].ToUpper().Contains(
                    "PARTIALLY_FILLED"))
            {
                dr["Color"] = Color.White.Name;
            }
            else if (ClsGlobal.DDReverseOrderStatus[Convert.ToSByte(item.OrderStatus)].ToUpper().Contains("WORKING"))
            {
            ...
            {
                dr["Color"] = Color.LimeGreen.Name;
            }
            dr["OrderQty"] = item.OrderQty;
            //dr["OrigClientOrderID"] = item.OrigClOrdId;
            dr["Price"] = item.Price; // / Math.Pow(10, digit);
            dr["Product"] = item.Product;
            dr["ProductType"] = pType;

            dr["Sector"] = item.sector;
            dr["Side"] = ClsGlobal.DDReverseSide[(sbyte)item.Side];
            dr["StopPx"] = item.StopPx;
            dr["TimeInForce"] = ClsGlobal.DDReverseValidity[Convert.ToSByte(item.TimeInForce)];
            //Namo 21 March
            //dr["TradeDate"] = clsTWSOrderManagerJSON.INSTANCE.GetDateTime(item.TransactTime);
            //dr["TransactTime"] = clsTWSOrderManagerJSON.INSTANCE.GetDateTime(item.TransactTime);

            Action A = () =>
                           {
                               clsTWSOrderManagerJSON.INSTANCE.orderHistoryDS.dtOrderHistory.Rows.Add(dr);
                               clsTWSOrderManagerJSON.INSTANCE.orderHistoryDS.dtOrderHistory.AcceptChanges();
                               //uctlOrderBook1.ui_uctlGridOrderBook.DataSource = orderHistoryDS.dtOrderHistory;
                           };
            if (InvokeRequired)
            {
                BeginInvoke(A);
        ...
        void uctlOrderBook1_HandleCloseOrderClick(object arg1, System.EventArgs arg2)
        {
            if (uctlOrderBook1.ui_uctlGridOrderBook.Rows.Count > 0)
            {
                ...
                if (orderStatus == "FILLED")
                {
                    ...
                    sbyte ordType = ClsGlobal.DDOrderTypes[uctlOrderBook1.ui_uctlGridOrderBook.SelectedRows[0].Cells["clmOrderType"].Value.ToString()];
                    sbyte side = ClsGlobal.DDSide[uctlOrderBook1.ui_uctlGridOrderBook.SelectedRows[0].Cells["clmBS"].Value.ToString()];
                    ...
                    sbyte Producttype = ClsGlobal.DDProductTypes[pType];
                    uint ordQty = Convert.ToUInt32(uctlOrderBook1.ui_uctlGridOrderBook.SelectedRows[0].Cells["clmOrderQuantity"].Value.ToString());
                    double price = Convert.ToDouble(uctlOrderBook1.ui_uctlGridOrderBook.SelectedRows[0].Cells["clmPrice"].Value.ToString());
                    double stopPx = Convert.ToDouble(uctlOrderBook1.ui_uctlGridOrderBook.SelectedRows[0].Cells["clmTriggerPrice"].Value.ToString());
                    sbyte tif = ClsGlobal.DDValidity[uctlOrderBook1.ui_uctlGridOrderBook.SelectedRows[0].Cells["clmValidity"].Value.ToString()];
