    ...
    class clsTWSOrderManagerJSON
    {
                    ...
                    foreach (OrderHistory ordH in lstOrderHistory)
                    {
                        ...
                        if (!_orderId.Contains(Convert.ToInt32(ordH.OrderID)))
                        {
                            ...
                            if (ClsTWSContractManager.INSTANCE.ddContractDetails.ContainsKey(ordH.Contract))
                            {
                                ...
                                if (!Cls.ClsGlobal.SubscibedSymbolList.ContainsKey(sym.Contract))
                                {
                                    Cls.ClsGlobal.SubscibedSymbolList.Add(sym.Contract, sym);
                                    if (!lst.Contains(sym))
                                    {
                                        lst.Add(sym);
                                    }
                ...
                try
                {
                    lock (objLock)
                    {
                        ...
                        if (executionReport.ExecType == 'I')
                        {
                            ...
                            try
                            {
                                if (tradeHistoryDS != null)
                                    tradeRow = tradeHistoryDS.dtTradeHistory.Select("OrderNo = '" + executionReport.ClOrdId + "'");
                                else
                                {
                                    tradeHistoryDS = new DS4TradeHistory();
                                    tradeRow = tradeHistoryDS.dtTradeHistory.Select("OrderNo = '" + executionReport.ClOrdId + "'");
                        ...
                        lock (orderHistoryDS.dtOrderHistory)
                        {
                            ...
                            if (!Cls.ClsGlobal.SubscibedSymbolList.ContainsKey(executionReport.Contract))
                            {
        ...
        public bool SubmitNewOrder(ClsNewOrder newOrder)
        {
            ...
            try
            {
                if (Convert.ToUInt32(Properties.Settings.Default.DefaultTradingAccount) != 0 && IsOrderMgrLoaded && clsTWSDataManagerJSON.INSTANCE.IsDataMgrConnected)
                {
                    ...
                    string message = string.Empty;
                    string side = Cls.ClsGlobal.DDReverseSide[Convert.ToSByte(newOrder.Side)];
                    string orderType = Cls.ClsGlobal.DDReverseOrderType[Convert.ToSByte(newOrder.OrderType)];
        ...
        internal bool ModifyOrder(ClsModifyOrder objModifyOrder)
        {
            try
            {
                ...
                {
                    string message = string.Empty;
                    string side = Cls.ClsGlobal.DDReverseSide[Convert.ToSByte(objModifyOrder.OldSide)];
                    string orderType = Cls.ClsGlobal.DDReverseOrderType[Convert.ToSByte(objModifyOrder.OldOrderType)];
                    Color color = Color.White;
        ...
        internal TradeAccount.LoginCredent CreateDemoAccount(string firstName, decimal balence, string country, string email, string leverage, string currencyName, bool isTradeEnable)
        {
            ...
            objPersonalInfo.FKAccountGroupID = 64;
            ...
            objPersonalInfo.WhiteLevelName = "LTECH";
            objPersonalInfo.IP_Name = "ECX";//This is actually LP Name
