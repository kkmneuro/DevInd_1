...
namespace PALSA.Cls
{
    class clsTWSDataManagerJSON
    {
        ...
        internal List<OHLC> GetOHLC(DateTime enddate, string symbol, int interval, string noOfRecord, int periodicity)
        {
            List<OHLC> lst = new List<OHLC>();
            LstOHLC lstOhlc = new LstOHLC();
            try
            {
                // Namo
                //PALSA.OHLCservice.OHLCserviceClient ohlcClient = new PALSA.OHLCservice.OHLCserviceClient();
                //PALSA.OHLCservice.OHLCData[] ohlcData = ohlcClient.GetHistoricalData(DateTimeToUnixTimestamp(enddate).ToString(), "SCB", "480", "1", "2000");
                ////OHLCService.OHLCData[] ohlcData = ohlcClient.GetHistoricalData("1490171731".ToString(), "SCB", "480", "1", "2000");
                ////PALSA.OHLCservice.OHLCData[] ohlcData = ohlcClient.GetHistoricalData(DateTimeToUnixTimestamp(enddate).ToString(), symbol, interval.ToString(), noOfRecord, "1");
                //if (ohlcData.Length > 0)
                //{
                //    foreach (var item in ohlcData)
                //    {
                //        OHLC oh = new OHLC();
                //        oh._Close = item.Close;
                //        oh._High = item.High;
                //        oh._Low = item.Low;
                //        oh._OHLCTime = UnixTimeStampToDateTime(item.DateTime); //item.DateTime.ToString("yyyy-mm-dd-hh-mm-ss");
                //        oh._Open = item.Open;
                //        long.TryParse(item.Volume.ToString(), out oh._Volume);
                //        lst.Add(oh);
                //    }
                //}
        ...
        public void SubscribeForQuotes(Enum ReqType, List<Symbol> lst)
        {
            try
            {
                if (websocket!=null && websocket.State== WebSocketState.Open && lst.Count > 0)
                {
                            ...
                            foreach (DataRow rw in rws)
                            {
                                ...
                                if (BuyQty > 0)
                                {
                                    if (Convert.ToDouble(ClsGlobal.GetZeroLevelBidPrice(rw["Contract"].ToString())) > 0)
                                    {
                                        ...
                                        {
                                            rw["UR_PL"] = Math.Round(BuyQty * (price - buyAvg) * ClsGlobal.DDContractSize[rw["Contract"].ToString()] * Cls.ClsGlobal.DDConversion[rw["Contract"].ToString()], 2);
                                        }
                                    }
                                }
                                else if (SellQty > 0)
                                {
                                    if (Convert.ToDouble(ClsGlobal.GetZeroLevelAskPrice(rw["Contract"].ToString())) > 0)
                                    {
                                        ...
                                        {
                                            rw["UR_PL"] = Math.Round(SellQty * (sellAvg - price) * ClsGlobal.DDContractSize[rw["Contract"].ToString()] * Cls.ClsGlobal.DDConversion[rw["Contract"].ToString()], 2);
                                        }
                                    }
                                }
                            }
    ...
    {
        public uint _MarketLevel;
        public double _Price;
        public QuoteStreamType _quoteType;
        public long _Size;
        public QuoteItemStatus _status;
        public string _Time;
