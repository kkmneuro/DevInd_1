...
//date_default_timezone_set("America/Los_Angeles");
	...
	try
	{
		...
		//$webURL		= "http://192.168.2.84:83/STTCservice.svc?wsdl&".rand(1,10000);
...
$objAGAC = new AccountGroupAssetClassMapping();
		...
		if(count($_POST>0) && isset($_POST['GetOpenTrades']))
		{
			...
			$param_GetOpenTrades=array('FromDate'=>$FromDate.'T00:00:00','ToDate'=>$ToDate.'T23:59:59','Login'=>$Login,'Password'=>$Password);
		...
		$testG = $GAGMwebSListE['lstAssetClassDetails']['AccountGroupAssetClassMapping'];
		if(isMultiArray($testG)){
			echo 'Yes';
			foreach($testG as $val)
			{
				
				//echo"<pre>"; print_r($val); //die;
				echo '<br>'.$val['AssetClassId'];
				echo '<br>'.$val['Prefix'];
				echo '<br>'.$val['Suffix'];
			}
		} else {
			echo 'No';
				echo '<br>'.$testG['AssetClassId'];
				echo '<br>'.$testG['Prefix'];
				echo '<br>'.$testG['Suffix'];
		}*/
		
		
		//echo"<pre>"; print_r($testG); //die; 
