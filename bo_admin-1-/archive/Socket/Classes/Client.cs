...
namespace DSSocket.Classes
{
    public class Client : IDisposable
    {
        ...
        readonly DataType DataType4Client;
        ...
        public Client()
        {
            DataType4Client = DataType.StringType; //DataType4Client_;

            Id = Guid.NewGuid().ToString();// "NEW CLIENT";
        ...
        public void Connect(IPAddress ipAddress, int port)
        {
            ...
            {
                FileHandling.WriteToLogEx(exception);
            ...
            lock (RecvSync)
            {
                ...
                try
                {
                    ...
                    if (error != SocketError.Success)
                    {
                        ...
                        Trace.WriteLine(string.Format("[SocketClient] BeginReceive finished with error '{0}'", error));
                    }
                }
                catch (Exception exception)
                {
        ...
        private void Receive_CB(IAsyncResult result)
        {
            if (!EndReceive(result))
            {
                ...
                Trace.WriteLine("[Client Socket] EndReceive said NO. Can't Receive more.");
                return;
            }
            StartReceiving();
        }
        ...
        public bool EndReceive(IAsyncResult result)
        {
            lock (RecvSync)
            {
                ...
                if (client.Disposed || client.IsClosed)
                {
                    ...
                    Trace.WriteLine(string.Format("[SocketClient] Can't EndReceive struct cause Disposed='{0}' or Close='{1}'", Disposed, IsClosed));
                    return false;
                }
                try
                {
                    ...
                    if (socketError != SocketError.Success)
                    {
                        ...
                        Trace.WriteLine(string.Format("[SocketClient] Could not receive. Error = '{0}'", socketError, read));
                        return false;
                    }
                    if (read <= 0)
                    {
                        //FileHandling.WriteToLogEx("[SocketClient] Received a negative quantity '{0}' " + read + "--" + socketError);
                        Trace.WriteLine(string.Format("[SocketClient] Received a negative quantity '{0}'", read, socketError));
                        return false;
                    }
                    //switch (DataType4Client)
                    //{
        ...
        public void BeginSend(IProtocolStruct structure, AsyncCallback callback)
        {
            lock (SendSync)
            {
                if (Disposed || IsClosed)
                {
                    ...
                    //                              IsClosed));
                    Trace.WriteLine(string.Format("Can't BeginSend cause Disposed='{0}' or Close='{1}'", Disposed, IsClosed));
                    this.Close();
                    return;
                }
                ...
                try
                {
                    ...
                    if (error != SocketError.Success)
                    {
                        ...
                        Trace.WriteLine(string.Format("[SocketClient] Can't begin cause of error '{0}'", error));
                    }
                }
                catch (Exception exception)
                {
            ...
            lock (SendSync)
            {
                ...
                try
                {
                    ...
                    if (socket == null)
                    {
                        ...
                        Trace.WriteLine("[SocketClient] Could not EndSend cause socket is Null");
                        return -1;
                    }
                    SocketError socketError;
                    if (socket == null)
                    ...
                    if (socketError != SocketError.Success)
                    {
                        ...
                        Trace.WriteLine(string.Format("[SocketClient] Could not send. Error = '{0}'", socketError));
                        return -1;
                    }
                    return res;
                }
        ...
        public void SendDataTable(IProtocolStruct @struct, Action onCompleted)
        {
        ...
        public void SendStruct(IProtocolStruct @struct, Action onCompleted)
        {
            BeginSend(@struct, result =>
            {
                EndSend(result);
        ...
        public void SendStruct(IProtocolStruct @struct)
        {
            Action onCompleted = () => { };
            BeginSend(@struct, result =>
            {
