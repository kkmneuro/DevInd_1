...
namespace DSSocket.Classes
{
    public class Server : IDisposable, IServerEvents
    {
        ...
        {
            dType4DataSending = DataType.BinaryType;// dType;
            IsStoped = false;
        ...
        private void DoListen(object state)
        {
            ...
            {
                Trace.WriteLine("EXCEPTION: TypeInitializationException : " + ex.Message);
            }
            catch (Exception ex)
            {
        ...
        void client_OnException(Client sender, Exception exception, bool? okToContinue)
        {
            if (ClientExceptionHandler != null)
                ClientExceptionHandler(sender, exception, true);
        }
        ...
        private void DoReceiveAsync(Client client)
        {
            ...
            {
                FileHandling.WriteToLogEx(ex);
                Trace.WriteLine("EXCEPTION: DoReceiveAsync : " + ex.Message);
                CloseClientSocket(client);
            }
        }
        ...
        public void Broadcast(ProtocolStructs.IProtocolStruct @struct)
        {
        ...
        //        Trace.WriteLine("EXCEPTION: DoSendAsync : " + ex.Message);
        //        CloseClientSocket(client);
        //    }
        //}
        ...
        private void ClientSend_CB(IAsyncResult result)
        {
            if (result.IsCompleted)
            {
                ...
                try
                {
                    ...
                    {
                        Trace.WriteLine(string.Format("[ServerSocket] Could not send data to client '{0}'. Closing it.", client.Id));
                        CloseClientSocket(client);
                        return;
                    }
                }
                ...
                {
                    FileHandling.WriteToLogEx(ex);
                    Trace.WriteLine("EXCEPTION: ClientSend_CB : " + ex.Message);
                    CloseClientSocket(client);
                    return;
                }
            }
