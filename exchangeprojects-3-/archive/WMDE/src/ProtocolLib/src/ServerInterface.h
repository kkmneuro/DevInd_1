...
public:
	...
	virtual int CheckHeartbeat() = 0;
	virtual void HandleRecdBuffer( char * buff
									, unsigned int size
									, IServerProtocolEvents* pServerProtEvents , 
									char *httpResponse
									, int& ResponseSize) = 0;
	...
	virtual bool WebsocketSend(void *pMsg, unsigned int MsgType, Socket *pServerSocket) = 0;
...
public:
	...
	virtual int GetMarkupPrice(symbol& sym, char side, bool bUp, unsigned long long price, unsigned long long& ulMarkedupPrice, int nGroupID, SymbolSpread& sp) = 0;
	virtual int GetSpreadInfo(symbol& sym, SymbolSpread& symSpread, int nGroupID) = 0;
	virtual int UpdateGlobalPosition(symbol& sym, char side, unsigned long filledPrice, unsigned long filledQty, char PositionEffect) = 0;
	virtual void GetGlobalPosition(symbol& sym, 
	...
	virtual void BroadcastToCCPs(PositionResponse *pResponse) = 0;
	virtual bool IsCCPsSubscribed() = 0;
	virtual IAccount *GetIAccountFromClOrderID(char *pszClOrdID) = 0;
	virtual int AddToClOrderIDIAccountMap(char *pszClOrdID, IAccount *pAccount) = 0;
	virtual int RemoveFromClOrderIDIAccountMap(char *pszClOrdID) = 0;
	virtual int LoadAllNonSettledMapping(IAccount *pAccount) = 0;

protected:
	virtual void AddAccount(char *ptrUserName, IClientSession *ptrSession
		, bool bIsCCP, ITable *ptrTable) =0;
};
...
public: 
	...
	virtual bool IsUserAlreadyLoggedIn(const char *pszUserName, unsigned int clientID) = 0;
	virtual bool StartListen() = 0;
	//virtual bool StartListen() = 0;
	//virtual bool IsUserAlreadyLoggedIn(const char *pszUserName, int ClientID) = 0;
};
