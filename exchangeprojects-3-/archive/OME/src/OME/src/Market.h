...
public:
	friend class TradeMarketOrder;
	Market(IDatabase* pDatabase, IDatabase *pDatabaseBO, Symbol sym, double lfDPR);
	virtual ~Market();
	...
	void UpdateOHLC(unsigned int uOpen, unsigned int uHigh, unsigned int uLow, unsigned int uClose, unsigned int uLTP, unsigned int uVol);
	bool SetDPR(double lfDPR);
	int MatchAllOrders();
};
...
private:
	...
	void UpdateOrder(IMarketOrderRequest* mOrder,ILimitOrderRequest* existingOrder,unsigned long long& qntyExecuted);
	void UpdateOrder(ILimitOrderRequest* mOrder, ILimitOrderRequest* existingOrder, unsigned long long& qntyExecuted);
	void MatchOrder(LIMIT_ORDER_REQUEST_LIST& limitOrderList,unsigned long long price, unsigned long long& qntyExecuted);
	void MatchOrder(LIMIT_ORDER_REQUEST_LIST& limitOrderList,IMarketOrderRequest* mOrder, unsigned long long& qntyExecuted);
	void MatchOrder(LIMIT_ORDER_REQUEST_LIST& limitOrderList, ILimitOrderRequest* mOrder, unsigned long long& qntyExecuted);
	...
	// BR 28 Aug 2016
	void ExecuteBuyOrder(ILimitOrderRequest* mOrder);
	void ExecuteSellOrder(ILimitOrderRequest* mOrder);

public:
	TradeMarketOrder(Market* pMarket);
	~TradeMarketOrder();
	virtual void ExecuteMarketOrder(IMarketOrderRequest* mOrder);
	virtual void ExecuteMarketOrder(ILimitOrderRequest* mOrder);
