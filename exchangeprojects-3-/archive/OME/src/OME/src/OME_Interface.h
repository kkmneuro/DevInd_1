...
public:
	...
	virtual void ExecuteMarketOrder(ILimitOrderRequest* mOrder) = 0;
...
public :
	...
	virtual void UpdateOHLC(unsigned int uOpen, unsigned int uHigh, unsigned int uLow, unsigned int uClose, unsigned int uLTP, unsigned int uVol) = 0;

	virtual bool SetDPR(double lfDPR) = 0;
	virtual int MatchAllOrders() = 0;
};
class GatewayClient;
class IOrderHandler
{
public:
	...
	virtual void HandleReloadDPR(ReloadDPR *pRequest) = 0; 
};
