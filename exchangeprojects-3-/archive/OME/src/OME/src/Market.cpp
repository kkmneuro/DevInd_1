...
//	Date			Initials	Desc
//
//  07 Feb 2014		BR			Ticket TradingApplication/112. Added function UpdateOHLC to load OHLC values and LTP while loading up.
//
///////////////////////////////////////////////////////////////////////////////////////////////////////////
...
Market::Market(IDatabase* pDatabase, IDatabase *pDatabaseBO, Symbol sym, double lfDPR)
{
//	stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "Market::Market, Entered");	
	...
	InitializeCriticalSection(&CS_OHLC);
...
void Market::UpdateOHLC(unsigned int uOpen, unsigned int uHigh, unsigned int uLow, unsigned int uClose, unsigned int uLTP, unsigned int uVol)
{
	...
	m_OHLCStats.LastPrice = uLTP;
	m_OHLCStats.LastSize = uVol;
...
void TradeMarketOrder::UpdateOrder(IMarketOrderRequest* mOrder,ILimitOrderRequest* existingOrder,unsigned long long& qntyExecuted)
{
	/*double lfDRP = m_ptrMarket->m_lfDPR;

	double lfDiff = m_ptrMarket->m_OHLCStats.LastPrice * lfDRP / 100.0;

	double lfUpperBand = m_ptrMarket->m_OHLCStats.LastPrice + lfDiff;
	double lfLowerBand = m_ptrMarket->m_OHLCStats.LastPrice - lfDiff;*/

	bool bOutBound = false;
	
	/*if (existingOrder->GetSide() == SIDE_BUY)
	{
		if (existingOrder->GetPrice() < lfLowerBand)
		{
			bOutBound = true;
		}
	}
	else if (existingOrder->GetSide() == SIDE_SELL)
	{
		if (existingOrder->GetPrice() > lfUpperBand)
		{
			bOutBound = true;
		}
	}*/
...
void TradeMarketOrder::UpdateOrder(ILimitOrderRequest* mOrder, ILimitOrderRequest* existingOrder, unsigned long long& qntyExecuted)
{
	/*double lfDRP = m_ptrMarket->m_lfDPR;
...
void TradeMarketOrder::MatchOrder(LIMIT_ORDER_REQUEST_LIST& limitOrderList,IMarketOrderRequest* mOrder, unsigned long long& qntyExecuted)
{
	...
	while(!limitOrderList.empty() && limitOrderIter!=limitOrderList.end())
	{
		...
		UpdateOrder(mOrder,existingOrder,executedQ);
...
void TradeMarketOrder::MatchOrder(LIMIT_ORDER_REQUEST_LIST& limitOrderList, ILimitOrderRequest* mOrder, unsigned long long& qntyExecuted)
{
	LIMIT_ORDER_REQUEST_LIST_ITER templimitOrderIter;
	LIMIT_ORDER_REQUEST_LIST_ITER limitOrderIter = limitOrderList.begin();
	...
	if(m_ptrMarket)
	{
		...
		while(marketOrderIter!=marketOrderList.end())
		{
			...
			if(limitOrder)
			{
				...
				// No price to be checked. Just fill the order at the price 
				if (mOrder->m_remainingQnty > 0)
				{
					MatchOrder(limitOrder->m_limitOrderList,mOrder,executedQ);
	...
	if(m_ptrMarket)
	{
		...
		while(marketOrderIter!=marketOrderList.end())
		{
			...
			if(limitOrder)
			{
				...
				{
					MatchOrder(limitOrder->m_limitOrderList,price,executedQ);
...
// BR 28 Aug 2016
void TradeMarketOrder::ExecuteBuyOrder(ILimitOrderRequest* mOrder)
{
	if (m_ptrMarket)
	{
		MARKET_ORDER_LIST& marketOrderList = m_ptrMarket->m_askList;
		...
		while (marketOrderIter != marketOrderList.end())
		{
			...
			if (limitOrder)
			{
				...
				// No price to be checked. Just fill the order at the price 
				if (mOrder->GetPrice() >= limitOrder->GetPrice() &&  mOrder->m_remainingQnty > 0)
				{
					MatchOrder(limitOrder->m_limitOrderList, mOrder, executedQ);
...
// BR 28 Aug 2016
void TradeMarketOrder::ExecuteSellOrder(ILimitOrderRequest* mOrder)
{
	if (m_ptrMarket)
	{
		MARKET_ORDER_LIST& marketOrderList = m_ptrMarket->m_bidList;
...
// BR 28 Aug 2016
void TradeMarketOrder::ExecuteMarketOrder(ILimitOrderRequest* mOrder)
{
...
void TradeMarketOrder::ExecuteOrders()
{
	if(m_ptrMarket)
	{
		EnterCriticalSection(&m_ptrMarket->CS_BID_LIST);
		...
		while (marketOrderIter != marketOrderList.end() && !isErased)
		{
			isErased = false;
			MarketOrder* limitOrder = dynamic_cast<MarketOrder*>(*marketOrderIter);
			...
			if (limitOrder)
			{
				EnterCriticalSection(&limitOrder->CS_LIMIT_ORDER_LIST);

				LIMIT_ORDER_REQUEST_LIST& limitOrderList = limitOrder->GetLimitOrderList();

				LIMIT_ORDER_REQUEST_LIST::iterator iter = limitOrderList.begin();

				while (iter != limitOrderList.end())
				{
					ILimitOrderRequest *mOrder = *iter;

					if (mOrder)
					{
						unsigned long long Volume1 = mOrder->m_remainingQnty;
						...
						if (mOrder->m_remainingQnty == 0)
						{
							...
							iter = limitOrderList.erase(iter);
						}
						else
						{
							isErased = true;
						...
						limitOrder->m_volumn -= (Volume1 - (mOrder->m_remainingQnty));
				...
				LeaveCriticalSection(&limitOrder->CS_LIMIT_ORDER_LIST);
			}
		}
						...
						bool bRequote)
{
	...
	if (MarketOrder)
	{
		...
		else if (MarketOrder->m_remainingQnty==0)
		{
	...
	TriggerStopOrders(LimitOrder->GetPrice(), SIDE_SELL);
	TriggerStopOrders(LimitOrder->GetPrice(), SIDE_BUY);
...
int Market::SendTradeReport(ILimitOrderRequest* LimitOrder,
	ILimitOrderRequest *MarketOrder,
	unsigned long long Qty,
	unsigned long long LastPx,
	bool bRequote)
{
...
bool Market::SetDPR(double lfDPR)
{
	bool bSuccess = false;
...
void Market::SendNULLMarketData()
{
	...
	{
		strcpy(m_OHLCStats.MarketDepth[nCount].Gateway,"ECXS");
...
void Market::ExpireStopOrders(MARKET_ORDER_LIST& list1)
{
	...
	for (;iter != list1.end();iter++)
	{
		...
		EnterCriticalSection(&pMarketOrder->CS_STOP_ORDER_LIST);
		...
		LeaveCriticalSection(&pMarketOrder->CS_STOP_ORDER_LIST);
