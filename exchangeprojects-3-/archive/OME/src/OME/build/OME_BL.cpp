...
{
	m_orderHandler=new OrderHandler(this,m_connnetionMgr,m_connnetionMgrMDE, m_databaseMgr, m_DatabaseOME, NULL);
}
...
IRequest* OME_BL::getIRequestPointer(MESSAGE msg)
{
	...
	else if (ptrHeader->MessageType == QUOTE__REQUEST)
	{
		reqMsg=new SnapshotRequest(msg.msg,m_orderHandler, ptrHeader->MessageType);
	}
	...
	else if (ptrHeader->MessageType == RELOAD_DPR)
	{
		reqMsg = new SnapshotRequest(msg.msg, m_orderHandler, ptrHeader->MessageType);
	}
	else
