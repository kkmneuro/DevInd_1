...
Database::Database()
{
	...
	sprintf_s(m_errStr, BUFF_SIZE - 1, "NULL POINTER");
...
bool Database::Open(const char* userName, const char* pwd, const char* cnnStr)
{
	...
	sprintf_s(m_errStr, BUFF_SIZE - 1, "Success");
	return true;
}
...
void Database::CloseDBConnection()
{
	...
	sprintf_s(m_errStr, BUFF_SIZE - 1, "DB Close");
	return;
}
...
bool Database::fnc_BeginTransaction()
{
	...
	sprintf_s(m_errStr, BUFF_SIZE - 1, "BeginTransaction Success");
	return true;
}
...
bool Database::fnc_CommitTransaction()
{
	...
	sprintf_s(m_errStr, BUFF_SIZE - 1, "Commit Success");
	return true;
}
...
bool Database::fnc_RollBackTransaction()
{
	...
	sprintf_s(m_errStr, BUFF_SIZE - 1, "Rollback Success");
	return true;
}
...
bool Database::OpenTable(char* cmdStr, ITable& tbl)
{
	...
	{
		sprintf_s(m_errStr, BUFF_SIZE - 1, "Invalid Connection");
		return false;
	}
...
bool Database::ReConnect()
{
	try
	{
		...
		Open(m_userName.c_str(), m_pwd.c_str(), m_cnnStr.c_str());
		//HRESULT hr = m_connectionPtr->Open(m_cnnStr.c_str(), m_userName.c_str(), m_pwd.c_str(), NULL);
	}
	catch (_com_error &e)
	{
		ErrorHandler(e, m_errStr);
...
bool Database::Execute(char* cmdStr, ITable& tbl, IParameter& prm)
{
	...
	nSuccess = ExecuteIn(cmdStr, tbl, prm);
...
int Database::ExecuteIn(char* cmdStr, ITable& tbl, IParameter& prm)
{
	...
	{
		if (e.Error() == E_FAIL || e.Error() == E_UNEXPECTED)
		{
			// Do a reconnect
			if (ReConnect())
			{
			...
			{
				ErrorHandler(e, m_errStr);
				stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "Database::Execute, error = %s. Could not reconnect", m_errStr);
