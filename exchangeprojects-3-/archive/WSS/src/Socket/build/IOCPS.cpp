...
IOCPS::IOCPS()
{
	...
	InitializeCriticalSection(&m_JobQueueLock);
	InitializeCriticalSection(&m_WorkerThreadMapLock);
...
BOOL IOCPS::Startup()
{
	...
	if ( m_iWSAInitResult != NO_ERROR )
	{
		//stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "IOCPS::Startup, Error at WSAStartup, Winsock 2.0 is not supported. Supported only in WinXP, Win2k and WinNT");
		return FALSE;	
	}
	...
	if ( !m_bServerStarted )
	{
		...
		*/
		...
		if ( m_bSendInOrder )
		{
			//stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_DEBUG, "IOCPS::Startup, Send ordering initialized. (Decreases the performance by ~3%)");
		}

		if ( m_bReadInOrder )
		{
			//stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_DEBUG, "IOCPS::Startup, Read ordering initialized. (Decreases the performance by ~3%)");
		}
		...
		if ( m_nPortNumber>0 )
		{
			...
			if( bRet )
			{
				//stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_DEBUG, "IOCPS::Startup, Connection listner thread successfully started.");
			}
		}
		// Setup the IOWorkers.. 
		bRet&=SetupIOWorkers();
		...
		if ( bRet )
		{
			//stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_DEBUG, "IOCPS::Startup, Successfully started %i Input Output Worker thread(s).", m_nIOWorkers);
		}
		...
		if ( bRet )
		{
			//stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_DEBUG, "IOCPS::Startup, Successfully started %i Input logical Worker thread(s).", m_nOfWorkers);
		}
	...
	if ( bRet )
	{
		if ( m_nPortNumber>0 )
		{
			//stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_DEBUG, "IOCPS::Startup, Waiting for clients on adress: %s, port:%i.", GetHostIP(),m_nPortNumber);
		}
		else 
		{
			//stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_DEBUG, "IOCPS::Startup, Client successfully started");
		}
	}
...
UINT IOCPS::ListnerThreadProc(LPVOID pParam)
{
	...
	if ( pThis )
	{
		while ( !pThis->m_bShutDown )
		{
			...
			{
				stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "IOCPS::ListnerThreadProc, WSAEnumNetworkEvents error %ld",WSAGetLastError());
				break;
			}
...
//#endif
					...
					if (clientSocket == SOCKET_ERROR)
					{
						...
						{
							stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "IOCPS::ListnerThreadProc, WWsaaccept() error: %s",pThis->ErrorCode2Text(WSAGetLastError()));
						}
					}else
				...
				{
					stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "IOCPS::ListnerThreadProc, Unknown network event error %ld",WSAGetLastError());
					break;
...
UINT IOCPS::IOWorkerThreadProc(LPVOID pParam)
{
	...
	if ( pThis )
	{
		...
		while ( !bError )
		{
			...
			{
				stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "IOCPS::IOWorkerThreadProc, ClientContext is NULL");
				bError=true;
			}
...
{	
	...
	// 	
	EnterCriticalSection(&m_ContextMapLock);// Mus lock the m_ContextMapLock Protect (m_NumberOfActiveConnections) ??
	...
	if (clientSocket == INVALID_SOCKET)
	{
		stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "IOCPS::AssociateIncomingClientWithContext, Invalid Socket");
		return FALSE; 
	}
	...
	if(pContext!=NULL)
	{
		...
		*/
		...
		if (nErr == -1)
		{
			...
			stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "IOCPS::AssociateIncomingClientWithContext, setsockopt() error: %s", ErrorCode2Text(WSAGetLastError()));
			return FALSE;
		}
		...
		{	
			...
			{
				stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "IOCPS::AssociateIncomingClientWithContext, Failed: %s", ErrorCode2Text(WSAGetLastError()));
			...
			if(pOverlapBuff!=NULL)
			{
				...
				{            
					...
					DisconnectClient(pContext);
					stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "IOCPS::AssociateIncomingClientWithContext, Failed PostQueuedCompletionStatus");
					ReleaseClientContext(pContext);
					return FALSE;
				}
			}
			...
			{
				stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "IOCPS::AssociateIncomingClientWithContext, Could not allocate memory");
				DisconnectClient(pContext);
				ReleaseClientContext(pContext);
				return FALSE;
			}
		...
		{
			stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "IOCPS::AssociateIncomingClientWithContext, ERROR HASHMAP AddClientContext failed.  %s", ErrorCode2Text(WSAGetLastError()));
			return FALSE;
		}
	}else
	{
		stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "IOCPS::AssociateIncomingClientWithContext, ERROR Could not allocate memory for Context in Acceptincoming:.  %s", ErrorCode2Text(WSAGetLastError()));
		closesocket(clientSocket);
		return FALSE;
	}
...
BOOL IOCPS::AddClientContext(ClientContext *mp)
{
	...
	{
		stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "IOCPS::AddClientContext, Dublicate Key! Disconnecting incoming client");
		AbortiveClose(mp);
		LeaveCriticalSection(&m_ContextMapLock);
		return FALSE;
	}
...
void IOCPS::FreeClientContext()
{
	...
	std::map<unsigned int, ClientContext* >::iterator pos = m_ContextMap.begin();
	while (pos != m_ContextMap.end()) 
	{
		unsigned int iKey;
		ClientContext *pContext=(*pos).second;
...
//#endif
			...
			stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_NORMAL, "IOCPS::FreeClientContext, Removed Client=%s", GetHostKeyName(iKey));
		}
	...
	std::list<ClientContext *>::iterator iter = m_FreeContextList.begin();
	while (iter != m_FreeContextList.end()) 
	{
		pContext = (*iter);
...
void IOCPS::OnRead(ClientContext *pContext,CIOCPBuffer *pOverlapBuff)
{
	...
	if(pContext&&!m_bShutDown)
	{
		if(pOverlapBuff==NULL)
		{
			...
			if(pOverlapBuff==NULL)
			{
				stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "IOCPS::OnRead, FAILED TO Allocate buffer memory in OnRead %s", ErrorCode2Text(WSAGetLastError()));
				DisconnectClient(pContext);
				ReleaseClientContext(pContext);
				return;
			}
...
#endif
						{
							stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "IOCPS::OnRead, Disconnect in Onread Possible Socket Error: %s", ErrorCode2Text(WSAGetLastError()));
						}
					}
					ReleaseBuffer(pOverlapBuff);
					DisconnectClient(pContext);
...
void IOCPS::OnPostedPackage(ClientContext *pContext, CIOCPBuffer *pOverlapBuff)
{
	if( !pContext )
	{
		stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "IOCPS::OnPostedPackage, FATAL ERROR OnPostedPackage pContext=NULL");
		return; 
	}
...
void IOCPS::OnReadCompleted(ClientContext *pContext, DWORD dwIoSize,CIOCPBuffer *pOverlapBuff)
{
	...
	{
		stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "IOCPS::OnReadCompleted, Disconnection in OnReadCompleted (dwIoSize == 0||pOverlapBuff==NULL) errcode: %s",ErrorCode2Text(WSAGetLastError()));
		DisconnectClient(pContext);
		ReleaseClientContext(pContext);
		ReleaseBuffer(pOverlapBuff);
		return;
...
//#endif
			...
			//NotifyReceivedPackage(pOverlapBuff, dwIoSize, pContext);
//#ifdef TRANSFERFILEFUNCTIONALITY
//			else
...
{
	//stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_DEBUG, "IOCPS::ProcessPackage, Entered");
	//
...
//	}
	...
	do 
	{
		...
		pPos = strstr((char *)pOverlapBuff->GetBuffer(), "\\r\\n\\r\\n");
		if (pPos != NULL)
		{
			done = true;
		}
...
#endif	
					{	
						stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "IOCPS::OnWrite, Error in OnWrite..: %s",ErrorCode2Text(WSAGetLastError()));
					}
				}
				ReleaseBuffer(pOverlapBuff);
				DisconnectClient(pContext);
		...
		if(pContext!=NULL)
		{
			...
			if(pOverlapBuff!=NULL)
			{
				...
				}else
				{
					ReleaseBuffer(pOverlapBuff);
					bRet&=FALSE;
					stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "IOCPS::ASendToAll, Buffer Error in  ASendToAll, AddData Failed: %s",ErrorCode2Text(WSAGetLastError()));
					break;	
				}
			}else
			{
				stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "IOCPS::ASendToAll, Could not allocate memory ASendToAll: %s",ErrorCode2Text(WSAGetLastError()));
...
std::string IOCPS::GetHostKeyName(SOCKET socket)
{
	sockaddr_in sockAddr;
...
std::string IOCPS::GetHostAddress(SOCKET socket)
{
	sockaddr_in sockAddr;
	memset(&sockAddr, 0, sizeof(sockAddr));
...
{	
	...
	{	
		...
		{		
			...
			std::map<unsigned int, ClientContext *>::iterator iter = m_ContextMap.find(pContext->m_Socket);
...
//				{
//					EnterCriticalSection(&m_OneIPPerConnectionLock);
//					m_OneIPPerConnectionList.remove(sockAddr.sin_addr.S_un.S_addr);
//					LeaveCriticalSection(&m_OneIPPerConnectionLock);	
//				}
//
//			}
//#endif	
...
{	
	BufferMap::iterator pos = map->begin ();
	while (pos != map->end()) 
	{
		unsigned int iKey;
...
void IOCPS::AbortiveClose(ClientContext *mp)
{
	...
	EnterCriticalSection(&m_FreeContextListLock);
	if(m_FreeContextList.size()<m_iMaxNumberOfFreeContext||m_iMaxNumberOfFreeContext==0)
		m_FreeContextList.push_front(mp);
	else
	{
		if(mp)
			delete mp;
...
//		EnterCriticalSection(&m_OneIPPerConnectionLock);
//		if(!m_OneIPPerConnectionList.empty())
//			m_OneIPPerConnectionList.clear();
//		LeaveCriticalSection(&m_OneIPPerConnectionLock);
//
//		EnterCriticalSection(&m_BanIPLock);
//		m_BanIPList.clear();
//		LeaveCriticalSection(&m_BanIPLock);
//#endif
	...
	if(pContext)
	{
		...
		if(pOverlapBuff==NULL)
		{
			...
			if(pOverlapBuff==NULL)
			{
				DisconnectClient(pContext);
				ReleaseClientContext(pContext);
				stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "IOCPS::OnZeroByteRead, FAILED TO Allocate buffer memory in OnZeroByteRead: %s",ErrorCode2Text(WSAGetLastError()));
				return; 
			}
		}
...
#endif	
				{
					stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "IOCPS::OnZeroByteRead, Disconnect in ZeroByteRead Possible Socket Error: %s",ErrorCode2Text(WSAGetLastError()));
				}
			}
			ReleaseBuffer(pOverlapBuff);
			DisconnectClient(pContext);			
...
BOOL IOCPS::AZeroByteRead(ClientContext *pContext, CIOCPBuffer *pOverlapBuff)
{
	...
	if(pContext->m_Socket!=INVALID_SOCKET )
	{
		...
		{            
			...
			DisconnectClient(pContext);
			ReleaseClientContext(pContext);
			stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "IOCPS::AZeroByteRead, Disconnect in AZeroByteRead Possible Socket Error: %s",ErrorCode2Text(WSAGetLastError()));
			return FALSE;
		}
...
std::string IOCPS::ErrorCode2Text(DWORD dw)
{
	std::string error="";
...
void IOCPS::DisconnectAll()
{
	...
	std::map<unsigned int, ClientContext *>::iterator pos = m_ContextMap.begin();
	while (!m_ContextMap.empty() &&pos != m_ContextMap.end()) 
	{
		unsigned int iKey;
		ClientContext *pContext=NULL;
		pContext = (*pos).second;
...
//		TRACE("Warning Buffer is still in use even if all users are gone. %i Buffers are inuse state. \r\n",nSize);
		...
		std::list<CIOCPBuffer *>::iterator pos = m_BufferList.begin();
		while (pos != m_BufferList.end()) 
		{
			pBuff = (*pos);
			if(pBuff)
			{
...
void IOCPS::ShutDownIOWorkers()
{
	...
	while(bIOWorkersRunning)
	{
		...
		std::list<HANDLE>::iterator pos = m_IOWorkerList.begin();
		while (pos != m_IOWorkerList.end()) 
		{
			HANDLE pThread = (*pos);
...
BOOL IOCPS::SetWorkers(int nThreads)
{
	...
	EnterCriticalSection(&m_WorkerThreadMapLock);
	int iNumberOfThreads=m_WorkerThreadMap.size();
	LeaveCriticalSection(&m_WorkerThreadMapLock);
	...
	EnterCriticalSection(&m_WorkerThreadMapLock);
	std::map<WORD, HANDLE>::iterator pos = m_WorkerThreadMap.begin();
	while (pos != m_WorkerThreadMap.end()&&iNumberToKill>0) 
	{
		WORD strKey;
		HANDLE pThread=NULL;
		pThread = (*pos).second;
		...
		if(pThread)
		{
			...
			::CloseHandle (pThread);
			iNumberToKill--;
			pos = m_WorkerThreadMap.erase(pos);
			delete[] pThread;
		}
	...
	{
		HANDLE pWorkerThread = (HANDLE)_beginthreadex( 0, 0,IOCPS::WorkerThreadProc, this, CREATE_SUSPENDED,  0 );
		//pWorkerThread->m_bAutoDelete=FALSE;  
		if(pWorkerThread)
		{
			ResumeThread(pWorkerThread);
			m_WorkerThreadMap.insert(std::pair<WORD, HANDLE>((WORD)pWorkerThread, pWorkerThread));
			iNumberToStart--;
		}else
		{
			return FALSE;
...
HANDLE IOCPS::GetWorker(WORD WorkerID)
{
	HANDLE pWorker=NULL;
	EnterCriticalSection(&m_WorkerThreadMapLock);
	pWorker=m_WorkerThreadMap[WorkerID];
	LeaveCriticalSection(&m_WorkerThreadMapLock);
...
void IOCPS::ShutDownWorkers()
{
	...
	JobItem *pJob=NULL;
	EnterCriticalSection(&m_JobQueueLock);
	std::list<JobItem *>::iterator pos = m_JobQueueList.begin();
	while (pos != m_JobQueueList.end()) 
	{
		pJob = (*pos);
		FreeJob(pJob);
	...
	m_JobQueueList.clear();
	LeaveCriticalSection(&m_JobQueueLock);
}
...
BOOL IOCPS::AddJob(JobItem *pJob)
{
	...
	if(m_bAcceptJobs&&pJob!=NULL)
	{
		EnterCriticalSection(&m_JobQueueLock);
		bRetur = TRUE;
		m_JobQueueList.push_front(pJob);
		LeaveCriticalSection(&m_JobQueueLock);
		//
		...
		if(bRetur)
		{
			EnterCriticalSection(&m_WorkerThreadMapLock);
			std::map<WORD, HANDLE>::iterator pos = m_WorkerThreadMap.begin();
			while (pos != m_WorkerThreadMap.end()) 
			{
				WORD strKey;
				HANDLE pThread= (*pos).second;
				if(pThread)
				{
					//if(pThread->ResumeThread()==1) // Some one wake up. 
					if (ResumeThread(pThread) == 1)
						break;
				}	
...
JobItem* IOCPS::GetJob()
{
	JobItem* pJob=NULL;
	EnterCriticalSection(&m_JobQueueLock);
	if(!m_JobQueueList.empty())
	{
		pJob=m_JobQueueList.back();
		m_JobQueueList.pop_back();
	}
	LeaveCriticalSection(&m_JobQueueLock);
...
ClientContext* IOCPS::AllocateContext()
{
	...
	{
		EnterCriticalSection(&m_FreeContextListLock);
		BOOL bGetFromBuff=m_FreeContextList.empty();

		if ( bGetFromBuff )
		{
			pContext=new ClientContext();
			InitializeCriticalSection(&pContext->m_ContextLock);
		}else
		{
...
CIOCPBuffer *IOCPS::AllocateBuffer(int nType)
{
	...
	int dummy=m_FreeBufferList.size();
	if(!m_FreeBufferList.empty())
	{
		pBuff=m_FreeBufferList.front();
	...
	if(pBuff==NULL)
	{
		...
		if( !pBuff )
		{ 
			stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "IOCPS::AllocateBuffer, Error new CIOCPBuffer(); failed in AllocateBuffer %s",ErrorCode2Text(WSAGetLastError()));
			return NULL;	
		}	
	}
...
BOOL IOCPS::ReleaseBuffer(CIOCPBuffer *pBuff)
{
	...
	//
	EnterCriticalSection(&m_FreeBufferListLock);
	if(m_iMaxNumberOfFreeBuffer==0||m_FreeBufferList.size()<m_iMaxNumberOfFreeBuffer)
	{
		m_FreeBufferList.push_front(pBuff);
	}else
	{
		// Delete the buffer. 
		if ( pBuff!=NULL )
...
void IOCPS::FreeBuffers()
{
	...
	CIOCPBuffer *pBuff=NULL;
	EnterCriticalSection(&m_FreeBufferListLock);
	std::list<CIOCPBuffer *>::iterator pos = m_FreeBufferList.begin();
	while (pos != m_FreeBufferList.end()) 
	{
		pBuff = (*pos);
		if(pBuff)
		{
...
BOOL IOCPS::SetupListner()
{
	...
	{
		stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "IOCPS::SetupListner, Could not create listen socket: %s",ErrorCode2Text(WSAGetLastError()));
		return FALSE;
	}
	...
	{	
		stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "IOCPS::SetupListner, WSACreateEvent() error : %s",ErrorCode2Text(WSAGetLastError()));
		closesocket(m_socListen);
		return FALSE;
	}
	...
	{
		closesocket(m_socListen);
		stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "IOCPS::SetupListner, WSAAsyncSelect() error: %s",ErrorCode2Text(WSAGetLastError()));
		return FALSE;
	}
	...
	{
		closesocket(m_socListen);
		stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "IOCPS::SetupListner, bind() error : %s",ErrorCode2Text(WSAGetLastError()));
		return FALSE;
	}
	...
	{
		closesocket(m_socListen);
		stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "IOCPS::SetupListner, listen() error: %s",ErrorCode2Text(WSAGetLastError()));
		return FALSE;
	}

	m_bAcceptConnections=TRUE;
	m_pListenThread = (HANDLE)_beginthreadex( 0, 0,IOCPS::ListnerThreadProc, this, 0, 0 );
	if ( !m_pListenThread )
	{
		stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "IOCPS::SetupListner, Error Could not start the ListnerThreadProc: %s",ErrorCode2Text(WSAGetLastError()));
		return FALSE;
	}
...
BOOL IOCPS::CreateCompletionPort()
{
	...
	if ( s == INVALID_SOCKET ) 
	{	
		stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "IOCPS::CreateCompletionPort, Error Could not start Create Socket in CreateCompletionPort: %s",ErrorCode2Text(WSAGetLastError()));
		return FALSE;
	}
	...
	{	
		closesocket( s );
		stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "IOCPS::CreateCompletionPort, CreateIoCompletionPort Failed: %s",ErrorCode2Text(WSAGetLastError()));
		return FALSE;
	}
...
BOOL IOCPS::SetupIOWorkers()
{
	...
	{
		pWorkerThread = (HANDLE)_beginthreadex( 0, 0,IOCPS::IOWorkerThreadProc, this, 0, 0 );
		if(pWorkerThread)
			m_IOWorkerList.push_front(pWorkerThread);
		else
		{
			stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "IOCPS::SetupIOWorkers, Error Couldnot start worker: %s",ErrorCode2Text(WSAGetLastError()));
			return FALSE;
		}
	}
...
*
*/
BOOL IOCPS::Connect(const std::string &strIPAddr, int nPort)
{
	if (m_bShutDown)
	{
		return FALSE; 
	...
	if (clientSocket == INVALID_SOCKET)
	{
		stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "IOCPS::Connect, Connect()->socket() Could not create  socket: %s",ErrorCode2Text(WSAGetLastError()));
		return FALSE;
	}
	// Clear the SockAddr. 
	memset(&SockAddr,0,sizeof(SockAddr));
	...
	keepAliveSettings.keepalivetime = 2500; 
	keepAliveSettings.keepaliveinterval = 1250;

	DWORD BytesReturned = 0;

	int lastError = WSAIoctl(clientSocket, 
					SIO_KEEPALIVE_VALS, 
					&keepAliveSettings, 
...
//			Logger::Error("Setting keepalive failed ErrCode %d\n",lastError); 
	...
	{
		closesocket(clientSocket);
		stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "IOCPS::Connect, Connection Error: "/*,ErrorCode2Text(WSAGetLastError()*/);
		return FALSE;
	}else
	{
...
//		TRACE("OutOforderRead: SS: %i, CurrentS: %i, pkg: %i\r\n",pContext->m_ReadSequenceNumber,pContext->m_CurrentReadSequenceNumber,iBufferSequenceNumber); 
		...
		// What if this fail?
		pContext->m_ReadBufferMap.insert(std::pair<unsigned int, CIOCPBuffer*>(iBufferSequenceNumber, pBuff));
		pRetBuff=NULL;
		//Changed 081220 
		...
		if(iter != pContext->m_ReadBufferMap.end())
		{
			...
			LeaveCriticalSection(&pContext->m_ContextLock);
			stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "IOCPS::GetNextReadBuffer, ERROR CAN NOT SAVE INTO BUFFER!!!");
			return NULL;
		}
	}
...
void IOCPS::MakeOrderdRead(ClientContext *pContext, CIOCPBuffer *pBuff)
{
	if (pContext!=NULL&&pBuff!=NULL)
	{
		...
		if ( nRetVal == SOCKET_ERROR && WSAGetLastError() != WSA_IO_PENDING) 
		{
			...
			if(WSAGetLastError()!=WSAENOTSOCK)
			{
				stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "IOCPS::MakeOrderdRead, Disconnect in Onread Possible Socket Error: %s",ErrorCode2Text(WSAGetLastError()));
			}
			//LeaveCriticalSection(&pContext->m_ContextLock);
			ReleaseBuffer(pBuff);
...
*/
//#if defined TRANSFERFILEFUNCTIONALITY
//BOOL IOCPS::PrepareSendFile(SOCKET clientSocket, std::string Filename)
//{
//	BOOL bRet=FALSE;
//	EnterCriticalSection(&m_ContextMapLock);
//	ClientContext* pContext=NULL;
...
void IOCPS::AppendLog(std::string msg)
{
}
...
//
//	//EnterCriticalSection(&m_OneIPPerConnectionLock);
//	//std::list<ULONG>::iterator pos=m_OneIPPerConnectionList.find(pCaller->sin_addr.S_un.S_addr);
//
//	//// If it is in the list return TRUE else add it to the list.. 
//	//if ( pos!=m_OneIPPerConnectionList.end() )
//	//{
//	//	LeaveCriticalSection(&m_OneIPPerConnectionLock);
//	//	return TRUE;
//	//}else
...
//	{
//		std::list<ULONG>::iterator pos=m_BanIPList.find(pVoid);
//		// The Client is in the banned list refuse
//		if ( pos!=m_BanIPList.end() )
//		{
//			LeaveCriticalSection(&m_BanIPLock);
//			return TRUE;
//		}
...
{
	//std::string sKey_PATH="\\SYSTEM\\CurrentControlSet\\Services\\Tcpip\\Parameters";
	//std::string sKey_SynAttackProtect="SynAttackProtect";
	//std::string sKey_TcpMaxHalfOpen="TcpMaxHalfOpen";
	//std::string sKey_TcpMaxHalfOpenRetried="TcpMaxHalfOpenRetried";
	//std::string sKey_TcpMaxPortsExhausted="TcpMaxPortsExhausted";
	//std::string sKey_TcpMaxConnectResponseRetransmissions="TcpMaxConnectResponseRetransmissions";
...
void IOCPS::RegisterEvents(ISocketEvents* pServer)
{
	m_pServerEvent=pServer;
}
