...
/*
* CreatePackage(std::string stxt)
* Creates one package in the buffe. (Se below) 
*
* [SizeHeader|...String..\0]
*/

BOOL CIOCPBuffer::CreatePackage(std::string stxt)
{
...
/*
* CreatePackage(BYTE Type,std::string stxt)
* Creates one package in the buffe. (Se below) 
*
* [SizeHeader|Type|...String..\0]
*/
BOOL CIOCPBuffer::CreatePackage(BYTE Type,std::string stxt)
{
	UINT nBufLen = stxt.length();	
...
BOOL CIOCPBuffer::CreatePackage(BYTE Type,UINT nData, char *stxt)
{
	UINT nBufLen = nData;
...
/*
* CreatePackage(BYTE Type, BYTE key, std::string stxt)
* Creates one package in the buffe. (Se below) 
*
* [SizeHeader|Type|Key|...String..\0]
*/
BOOL CIOCPBuffer::CreatePackage(BYTE Type, BYTE key, std::string stxt)
{
	UINT nBufLen = stxt.length();	
...
/*
* CreatePackage(BYTE Type,BYTE key1, BYTE key2, BYTE key3,std::string stxt)
* Creates one package in the buffe. (Se below) 
*
* [SizeHeader|Type|Key1|key2|key3|...String..\0]
*/
BOOL CIOCPBuffer::CreatePackage(BYTE Type, BYTE key1, BYTE key2, BYTE key3, std::string stxt)
{
	UINT nBufLen = stxt.length();	
...
// Reads the info from a package created with CreatePackage(std::string stxt);
BOOL CIOCPBuffer::GetPackageInfo(std::string &stxt)
{
...
// Reads the info from a package created with CreatePackage(BYTE Type,std::string stxt);
BOOL CIOCPBuffer::GetPackageInfo(BYTE& Type, std::string &stxt)
{
	int nSize=GetPackageSize();
	if ( nSize>=1 && nSize<=MAXIMUMPAYLOADSIZE )
	{
...
// Reads the info from a package created with CreatePackage(BYTE Type,std::string stxt);
BOOL CIOCPBuffer::GetPackageInfo(BYTE& Type,UINT& nData, char* stxt)
{
	int nSize=GetPackageSize();
	stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "CIOCPBuffer::GetPackageInfo:: Before Calling GetPAckageInfo %d", nSize);
	...
	//if (nSize > 0)
	{
		...
		PBYTE pData = GetBuffer()  ;
		// Assumes that we already have a null termination. 
		memmove(stxt,pData, m_nUsed);
...
BOOL CIOCPBuffer::GetPackageInfo(BYTE &Type, BYTE &key1, BYTE &key2, BYTE &key3, std::string &stxt)
{
	int nSize=GetPackageSize();
	if ( nSize>=1 && nSize<=MAXIMUMPAYLOADSIZE )
	{
...
BOOL CIOCPBuffer::GetPackageInfo(BYTE &Type, BYTE &key, std::string &stxt)
{
	int nSize=GetPackageSize();
	if ( nSize>=1 && nSize<=MAXIMUMPAYLOADSIZE )
	{
