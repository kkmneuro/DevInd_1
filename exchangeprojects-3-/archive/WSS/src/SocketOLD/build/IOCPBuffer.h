...
public:
	...
	BOOL GetPackageInfo(BYTE &Type, BYTE &key, std::string &stxt);
	BOOL CreatePackage(BYTE Type, BYTE key,  std::string stxt);
	BOOL GetPackageInfo(BYTE &Type,BYTE &key1,BYTE &key2,BYTE &key3, std::string &stxt);
	// Used with overlapped.. 
	OVERLAPPED			m_ol;
	BOOL Flush(UINT nBytesToRemove);
	//std::list<CIOCPBuffer *>::iterator GetPosition();
	//void SetPosition(std::list<CIOCPBuffer *>::iterator pos);    
	WSABUF * GetWSABuffer();
	...
	BOOL CreatePackage(std::string stxt);
	BOOL CreatePackage(BYTE Type,std::string stxt);
	BOOL CreatePackage(BYTE Type,UINT nData, char *stxt);
	BOOL CreatePackage(BYTE Type);
    BOOL CreatePackage(BYTE Type,BYTE key1,BYTE key2,BYTE key3, std::string stxt);
	//
	...
	BOOL GetPackageInfo(std::string &stxt);
	// Reads the info from a package created with CreatePackage(BYTE Type,std::string stxt);
	BOOL GetPackageInfo(BYTE &Type, std::string &stxt);
	// Reads the info from a package created with CreatePackage(BYTE Type,std::string stxt);
	BOOL GetPackageInfo(BYTE &Type,UINT &nData, char *stxt);
...
private:
	...
	//POSITION m_pPos;
	std::list<CIOCPBuffer *>::iterator m_pPos;
};
