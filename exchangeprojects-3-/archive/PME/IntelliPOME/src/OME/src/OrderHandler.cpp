...
void OrderHandler::InitializeMarket()
{
	...
	while (!tb->IsEOF())
	{
		...
		{
			IMarket *mkt = new Market(m_ptrDatabaseOME, m_ptrDatabase, tempSymbol);
...
void OrderHandler::InitializeOHLC()
{
	...
	if (tb)
	{
		...
		if (param)
		{
			bool isSPExist = m_ptrDatabase->Execute("OME_GetOHLCForCurrentDay",*tb,*param);//execute sp
			...
			while (!tb->IsEOF())
			{
				...
				stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "OrderHandler::InitializeOHLC, Symbol Loaded=%s, O:H:L:C=%u:%u:%u:%u", contract, uOpen, uHigh, uLow, uClose);
				...
				if (iter != m_MktMap.end())
				{
					...
					if (pMkt)
					{
						pMkt->UpdateOHLC(uOpen, uHigh, uLow, uClose);
					}
				}
