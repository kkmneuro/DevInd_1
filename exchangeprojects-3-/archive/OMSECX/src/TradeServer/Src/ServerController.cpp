...
{
	SendSignal(SERVER_OMS, SIGNAL_TYPE_STARTUP, SIGNAL_STATUS_STARTING);
	...
	SendSignal(SERVER_OMS, SIGNAL_TYPE_STARTUP, SIGNAL_STATUS_PORTS_READY);

	m_ptrIServerBL = CreateBusinessObject(m_connectionMgr);

	if (m_ptrIServerBL)
		m_ptrIServerBL->setConnectionMgr(m_connectionMgr);

	SendSignal(SERVER_OMS, SIGNAL_TYPE_STARTUP, SIGNAL_STATUS_BO_UP);
