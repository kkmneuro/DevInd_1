...
public:
	virtual int UpdateNetPosition(IOrder *pOrder) = 0;
	virtual int UpdateNetPosition(ExecutionReport *pReport) = 0;

	virtual bool GetPositionResponse(void*& pResponse) = 0;
	virtual bool GetPositionResponseEx(position *pResponse) = 0;
	...
	virtual int CancelPositionTags(IOrder *pOrder, ExecutionReport *pReport, void* lstOrdersToUpdate) = 0;
};