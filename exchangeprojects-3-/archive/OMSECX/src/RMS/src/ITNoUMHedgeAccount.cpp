...
#undef _this
#endif
#define _this "ITNoUMHedgeAccount"

ITNoUMHedgeAccount::ITNoUMHedgeAccount(unsigned int ulAccount,
				double lfBalance,
				int nLeverage,
				int nGroup,
				double lfFreeMargin,
...
ITNoUMHedgeAccount::~ITNoUMHedgeAccount()
{

}

// Checks if the margin is available or not.
int ITNoUMHedgeAccount::CheckMargin(IOrder *ord, unsigned long long Qty, double lfFees, double lfTax, IContractManager *pContractManager, IPosition *pPosition)
{
	//stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_DEBUG, "ITNoUMHedgeAccount::CheckMargin, Entered");
		...
		if (!FAILED(hr))
		{
			if (ord->m_Order.OrderType == ORDER_TYPE_MARKET_ORDER)
			{
				...
				if (!FAILED(hr) && ulCurrentPrice != 0)
				{
					...
					stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_DEBUG, "ITNoUMHedgeAccount::CheckMargin, Initial MArgin = %lu, CurrentPrice = %lf", (ULONG)initialMargin, currentPrice);
				}
				else
				{
			...
			if (nSuccess == 0)
			{
				...
				stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_DEBUG, "ITNoUMHedgeAccount::CheckMargin, Before Cost Required = %lf, Leverage=%d, Balance=%lf, used Margin=%lf", CostRequired, m_nLeverage, m_Balance, m_lfUsedMargin);
				...
				stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_DEBUG, "ITNoUMHedgeAccount::CheckMargin, After Cost Required = %lf, Leverage=%d, Balance=%lf, used Margin=%lf", CostRequired, m_nLeverage, m_Balance, m_lfUsedMargin);
			}
		}
		else
		{
			...
			stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_DEBUG, "ITNoUMHedgeAccount::CheckMargin, Error in Contract Manager. %s", errorText);
		}
	}
...
int ITNoUMHedgeAccount::ProcessExecutionReport(ExecutionReport *pReport, 
										IOrder *&pOrder, 
...
int ITNoUMHedgeAccount::ProcessCROrder(IOrder *ord, IOrder *pNewOrder, double fees, double tax, IContractManager *pContractManager)
{
	int nSuccess = 0;
...
double ITNoUMHedgeAccount::CalculateUsedMargin(IOrder *ord, unsigned long long Qty, IContractManager *pContractManager)
{
	double lfUsedMargin = 0;
...
int ITNoUMHedgeAccount::CalculatePnLAndLiquidate(IContractManager *pContractManager, IConnectionMgr *pConnMgr)
{
	int nSuccess = 0;
...
int ITNoUMHedgeAccount::LiquidateOrder(symbol& sym, IOrder *pOrder, IConnectionMgr *pConnMgr, unsigned long ulBid, unsigned long ulAsk)
{
	int nSuccess = 0;
