...
ITOneUMHedgePosition::ITOneUMHedgePosition(RMSCalculatorFactory* pCalcFactory)
{
	//stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_DEBUG, "ITOneUMHedgePosition::ITOneUMHedgePosition, Entered");
...
int ITOneUMHedgePosition::UpdatePositionStats(char side, unsigned long Qty, char positionEffect, unsigned long long price, unsigned long& CloseQty, double& lfProfit)
{
	int nSuccess = 0;

	stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_DEBUG, "ITOneUMHedgePosition::UpdatePositionStats, Enter Position=%s, CloseQty=%lu", ToString().c_str(), CloseQty);
	...
	stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_DEBUG, "ITOneUMHedgePosition::UpdatePositionStats, Exit Position=%s, CloseQty=%lu", ToString().c_str(), CloseQty);

	return nSuccess;
}

int ITOneUMHedgePosition::GetQtyToCheckForMargin(unsigned long& ulQty, IOrder *pOrder, bool bHedge)
{
	int nSuccess = 0;
	long position = 0;

	stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_DEBUG, "ITOneUMHedgePosition::GetQtyToCheckForMargin, Enter Position=%s, Qty=%lu", ToString().c_str(), ulQty);
	...
	stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_DEBUG, "ITOneUMHedgePosition::GetQtyToCheckForMargin, Exit Position=%s, Qty=%lu", ToString().c_str(), ulQty);

	return nSuccess;
}

//double ITOneUMHedgePosition::CalculateOpenPnL(double bid, double ask, bool& SessionClosed, double& tradeMargin, unsigned long& Qty, char& side, int leverage)
//{
//	double OpenPnL = 0;
//
//	if (!SessionClosed)
...
//}
//
void ITOneUMHedgePosition::ResetPosition(unsigned long pos, char side)
{
	if (side == SIDE_BUY)
	{
		m_ulTempLongPos -= pos;
...
int ITOneUMHedgePosition::CalculateProfit(IOrder *pOpenOrder, IOrder *pCloseOrder, ExecutionReport *pReport, unsigned long qty, double& lfProfit, double& lfUsedMargin)
{
	// Not used in this
	int nSuccess = 0;
...
int ITOneUMHedgePosition::UpdatePosition(IOrder *pOrder, ExecutionReport *pReport, double& lfProfit, double& lfUsedMargin, unsigned long& ullContractSize, void* lstOrdersToUpdate, IGlobalPosition *pGlobalPos)
{
	int nSuccess = 0;
...
int ITOneUMHedgePosition::AdjustUsedMargin(IOrder *pOrder, ExecutionReport *pReport, double lfUsedMargin, double& lAdjustmentDone)
{
	int nSuccess = 0;
	int qty = 0;
	...
	if (pOrder->m_Order.PositionEffect == POSITION_CLOSING_TRADE)
	{
		...
		if (m_ulBuySideUsedMargin > m_ulSellSideUsedMargin)
			overallMargin = m_ulBuySideUsedMargin;
		else
			overallMargin = m_ulSellSideUsedMargin;
...
int ITOneUMHedgePosition::UpdateUsedMargins(char side, double ulUM, double overAll, double& adjustment)
{
	int nSuccess = 0;
	...
	if (m_ulBuySideUsedMargin > m_ulSellSideUsedMargin)
		lfOverAllUsedMargin = m_ulBuySideUsedMargin;
	else 
		lfOverAllUsedMargin = m_ulSellSideUsedMargin;
...
int ITOneUMHedgePosition::CalculateOpenPnL(double bid, double ask, char *pclOrdID, double& lfMaxLoss, double& lfOpenPnL, unsigned long& ullContractSize, IOrder*& pOrder1)
{
	int nSuccess = -1;
