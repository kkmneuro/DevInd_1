...
//11-04-2014	BR					Ticket no ECX/135. Liquidation order was not going to OME/PME because of 
//									wrong gaetway. this came only when previous days position was being liquidated.
//									Modified SP OMS_GetListOfNonSettledOrderbyAccountID to return correct gateway
//									instead of OEC
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//=================================================================================================
// INCLUDE FILES
...
#undef _this
#endif
#define _this "ITNonHedgeAccount"

ITNonHedgeAccount::ITNonHedgeAccount(unsigned int ulAccount,
				double lfBalance,
				int nLeverage,
				int nGroup,
				double lfFreeMargin,
...
ITNonHedgeAccount::~ITNonHedgeAccount()
{

}

// Checks if the margin is available or not.
int ITNonHedgeAccount::CheckMargin(IOrder *ord, unsigned long long Qty, double lfFees, double lfTax, IContractManager *pContractManager, IPosition *pPosition)
{
	int nSuccess = 0;
	...
	if (!FAILED(hr))
	{
		if (ord->m_Order.OrderType == ORDER_TYPE_MARKET_ORDER)
		{
			...
			// We will just use the price that is coming from the platform.
			// this is not the market execution but the instant execution
			...
			if (!FAILED(hr) && ulCurrentPrice != 0)
			{
				...
				stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_DEBUG, "ITNonHedgeAccount::CheckMargin, Initial MArgin = %lu, CurrentPrice = %lf", (ULONG)initialMargin, currentPrice);
			}
			else
			{
				nSuccess = ERR_INTERNAL_ERROR;
				//stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_DEBUG, "ITNonHedgeAccount::CheckMargin, Not able to get latest price");
			}
...
//			double ActualCost = Qty * currentPrice;
			...
			stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_DEBUG, "ITNonHedgeAccount::CheckMargin, Before Cost Required = %lf, Leverage=%d, Balance=%lf, used Margin=%lf", CostRequired, m_nLeverage, m_Balance, m_lfUsedMargin);
			...
			stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_DEBUG, "ITNonHedgeAccount::CheckMargin, After Cost Required = %lf, Leverage=%d, Balance=%lf, used Margin=%lf", CostRequired, m_nLeverage, m_Balance, m_lfUsedMargin);
		}
	}
	else
	{
		...
		stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_DEBUG, "ITNonHedgeAccount::CheckMargin, Error in Contract Manager. %s", errorText);
	}
...
int ITNonHedgeAccount::ProcessExecutionReport(ExecutionReport *pReport, 
										IOrder *&pOrder, 
                    ...
                    IGlobalPosition *pGlobalPos)
{
	...
	//if (strcmp(pReport->ClOrdId, "49653") == 0)
	if (pReport->Account == 243)
	{
		int GG = 0;
	}
	...
	{
		stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_DEBUG, "ITNonHedgeAccount::ProcessExecutionReport, Order Not found. FATAL, Order ID = %s", pReport->ClOrdId);
		nSuccess = ERR_INTERNAL_ERROR;
	}
	...
	if (nSuccess == ERR_OK)
	{
		if (m_bRMSEnable)
		{
			...
			if (pPosition)
			{
				...
				{
					stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "ITNonHedgeAccount::ProcessExecutionReport Enter, Account Used Margin = %lf, Order Used Margin = %lf, Balance = %lf, Order ID = %s", m_lfUsedMargin, pOrder->m_lfUsedMargin, m_Balance, pOrder->m_Order.ClOrdId);
					double lfProfit = 0;
					...
					{	
						...
						if (nSuccess == 0)
						{
							stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "ITNonHedgeAccount::ProcessExecutionReport, Account Used Margin = %lf, Order Used Margin = %lf, Profit = %lf, Balance = %lf, Order ID = %s", m_lfUsedMargin, lfUsedMargin, m_Balance, lfProfit, pOrder->m_Order.ClOrdId);
							...
							pReport->Profit = lfProfit/ulContractSize;
							pPosition->UpdateNetPosition(pReport);
			...
			pReport->OrderStatus == ORDER_STATUS_PARTIALLY_FILLED)
		{
			if (pReport->OrderStatus != ORDER_STATUS_FILLED && pReport->OrderStatus != ORDER_STATUS_PARTIALLY_FILLED)
			{
				stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_ALWAYS, "ITNonHedgeAccount::ProcessExecutionReport 1, Account Used Margin = %lf, Order Used Margin = %lf, Balance = %lf, Order ID = %s", m_lfUsedMargin, pOrder->m_lfUsedMargin, m_Balance, pOrder->m_Order.ClOrdId);
				
				//m_Balance += pOrder->m_lfUsedMargin;
				double lfPrevCharges = pOrder->m_lfCommission + pOrder->m_lfTax;

				if (pOrder->m_Order.OrderQty == pOrder->QtyClose || pOrder->m_Order.OrderQty == pOrder->CumQty)
				{
			...
			else if (pReport->OrderStatus == ORDER_STATUS_CANCEL)
			{
				m_lstWorkingOrders.erase(iter);
			}
		}
		else if (pReport->OrderStatus == ORDER_STATUS_NEW)
		{
			if (pOrder->m_Order.origClOrdId[0] != '\0')
			{
				...
				{
					stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_DEBUG, "ITNonHedgeAccount::ProcessExecutionReport, Order Not found. FATAL");
					nSuccess = ERR_INTERNAL_ERROR;
				}
			}
		}
		...
		if (pPosition)
		{
			ulTempLongPos = 0;//pPosition->m_ulTempLongPos;
			ulTempShortPos = 0;//pPosition->m_ulTempShortPos;
		}
	}
...
int ITNonHedgeAccount::ProcessCROrder(IOrder *ord, IOrder *pNewOrder, double fees, double tax, IContractManager *pContractManager)
{
	int nSuccess = 0;
...
double ITNonHedgeAccount::CalculateUsedMargin(IOrder *ord, unsigned long long Qty, IContractManager *pContractManager)
{
	double lfUsedMargin = 0;
...
int ITNonHedgeAccount::CalculatePnLAndLiquidate(IContractManager *pContractManager, IConnectionMgr *pConnMgr)
{
	int nSuccess = 0;
	...
	if(m_lfUsedMargin!=0)
	{
		...
		stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_DEBUG, "ITNonHedgeAccount::CalculatePnLAndLiquidate, Balance = %lf, OpenPnL = %lf, UM = %lf,Equity = %lf, MArgin Level = %lf",
																	m_Balance,
																	m_OpenPnL,
																	m_lfUsedMargin,
																	lfEquity,
		...
		if (lfMarginLevel < m_nMarginCall3)
		{
			...
			stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_DEBUG, "ITNonHedgeAccount::LiquidateOrder");

			//LiquidateOrder(sym, pOrder1, pConnMgr, ulBidPrice1, ulAskPrice1);
		}
	}
	else 
		stl::log::trace(stl::log::LOG_GROUP_ERROR, stl::log::LOG_LEVEL_DEBUG, "ITNonHedgeAccount::CalculatePnLAndLiquidate m_lfUsedMargin =0.000000");
...
int ITNonHedgeAccount::LiquidateOrder(symbol& sym, IOrder *pOrder, IConnectionMgr *pConnMgr, unsigned long ulBid, unsigned long ulAsk)
{
	int nSuccess = 0;
