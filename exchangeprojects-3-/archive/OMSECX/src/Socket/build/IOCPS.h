...
#define MAXIMUMPACKAGESIZE 8192
...
//template<>
//inline UINT HashKey(unsigned int& key)
//{
//	// default identity hash - works for most primitive values
//	return ((UINT)(void*)(unsigned int)key) >> 4;
//}

typedef std::map<unsigned int, ClientContext* > ContextMap;
typedef std::map<unsigned int, CIOCPBuffer *> BufferMap;
...
//protected: 
//	CRITICAL_SECTION m_OneIPPerConnectionLock;
//	std::list<ULONG> m_OneIPPerConnectionList;
//	CRITICAL_SECTION m_BanIPLock;
//	std::list<ULONG> m_BanIPList;
//
...
//#endif
	...
	static UINT _stdcall WorkerThreadProc(LPVOID pParam);
	static UINT _stdcall IOWorkerThreadProc(LPVOID pParam);
	static UINT _stdcall ListnerThreadProc(LPVOID pParam);
	...
	std::list<HANDLE> m_IOWorkerList;
	...
	CRITICAL_SECTION m_JobQueueLock;
	std::list<JobItem *> m_JobQueueList;

	// We save our workers here. 	
	std::map<WORD, HANDLE> m_WorkerThreadMap;
	CRITICAL_SECTION m_WorkerThreadMapLock;
	// One ipPerConnection 
	...
	HANDLE GetWorker(WORD WorkerID);

	

public: 

	ISocketEvents*											m_pServerEvent;
...
//#if defined TRANSFERFILEFUNCTIONALITY
//
//	BOOL PrepareSendFile(SOCKET clientSocket, std::string Filename);
...
//#endif
	...
	std::string GetHostAddress(SOCKET socket);
	...
	BOOL Connect(const std::string& strIPAddr, int nPort);
	...
	void RegisterEvents(ISocketEvents* pServer);
	...
	std::string GetHostKeyName(SOCKET socket);	
protected:
...
#endif // !defined(AFX_ITIOCPSERVER_H__4D63F25E_B852_46D7_9A42_CF060F5E544D__INCLUDED_)
